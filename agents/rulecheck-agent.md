...
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "${CLAUDE_PLUGIN_ROOT}/skills/rulecheck/hooks/block-dangerous.sh"
          statusMessage: "Checking command safety..."
  Stop:
    - hooks:
        - type: command
          command: "${CLAUDE_PLUGIN_ROOT}/skills/rulecheck/hooks/slack-notify.sh"
...
Scan `src/**/*.ts` for violations of the rules you identified in
...
gh pr create --title "fix: [concise title]" --body "## What\n[describe the concern fixed]\n\n## Changes\n- [list each fix]"
```
...
- Patterns noticed (which files have the most violations, recurring issues)

**Important**: Include the memory files in the PR commit so they persist:

```bash
git add .claude/agent-memory/rulecheck-agent/
...
- **Validate after changes only** — `bun test && bun run type-check` after edits, not before
...
- [ ] Ran `bun test && bun run type-check` and it passed
