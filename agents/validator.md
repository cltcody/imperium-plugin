...
1. **Identify test framework**: Prefer the project's `STACK.md` (`test` step per `${CLAUDE_PLUGIN_ROOT}/references/dev/stack-resolution.md`); otherwise detect from package.json, requirements.txt, or project config
...
4. **Run tests**: Use the resolved `test` command from `STACK.md` (e.g. npm test, pytest), run from the component's `working_dir`
