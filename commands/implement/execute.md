...
2. **Resolve the stack.** Read the project's `STACK.md` and resolve commands per `${CLAUDE_PLUGIN_ROOT}/references/dev/stack-resolution.md`. You'll use the canonical steps (`smoke`, `test`, `typecheck`, `lint`, `format:check`, `migrate`) per component from each component's `working_dir`. If there is no `STACK.md`, auto-detect once from project markers and recommend the user run `/cc:setup:stack` to persist a manifest. Skip any step a component does not map (not an error).
