...
6. **Check PIV working dirs.** List `${user_config.workspace_dir}/plans/` and `${user_config.workspace_dir}/execution-reports/` if present — note any in-flight plans. List top-level only; ignore `${user_config.workspace_dir}/archive/` — it holds closed artifacts the PIV state detector already excludes.
