...
6. **Draft conventional-commit message(s).** Format: `<type>(<scope>): <imperative summary>` with an imperative-mood summary and type one of `feat | fix | docs | refactor | test | chore | perf | ci`. Add a short body explaining the why when the summary alone is not enough. Reference issues deliberately, not "where applicable": check which open issues the change completes (`Fixes #N`, one per completed issue) or merely advances (`Refs #N`) — work tracked under a plan or feature name still maps back to issues, and the PR body downstream inherits this enumeration.
...
8. **Commit.** After approval: if a commit completes the work of a plan in `${user_config.workspace_dir}/plans/`, first set that plan's header `Status:` line to `implemented` (adding the line within the first 5 lines if the plan predates the convention) and include the edit in the commit — the PIV state detector reads it to keep finished plans out of phase detection; skip when the commit is a partial step of a still-open plan. Then stage each group with explicit paths (`git add <files>` — never blanket `git add -A` when splitting) and commit. Verify with `git log --oneline -n <count>` and `git status` (tree should be clean or contain only intentionally excluded files). If the project configures pre-commit hooks (e.g. linters, type checkers, secret scanners), they run on every commit — if a hook fails, fix the underlying issue rather than bypassing it.
