...
Paste the coverage matrix and key responses from `/cc:rfp:respond`, or describe what was submitted. This command builds the polished presentation deck that makes a human case alongside the formal document response, is shown in a follow-up session (if the customer invites shortlisted vendors), and can serve as a standalone response for RFIs that ask for a deck rather than a document.
...
SECTION 1 — EXECUTIVE SUMMARY (1–2 slides) — give the evaluator the answer in the first 90 seconds.
...
SECTION 2 — UNDERSTANDING OF YOUR REQUIREMENTS (2–3 slides) — show we read the RFP and understood it, not just answered it.
...
SECTION 3 — OUR SOLUTION (3–4 slides) — map our products directly to their requirements.
  ...
  - Do NOT lead with product names — lead with requirements met.
SECTION 4 — IMPLEMENTATION APPROACH (2 slides) — show this is real and deliverable, not just a pitch.
...
SECTION 5 — VALUE CASE (1–2 slides) — make the financial case without overselling.
  ...
  - Mandatory: all figures confidence-tagged before this slide goes to a customer.
SECTION 6 — WHY ${user_config.company} (1–2 slides) — one defensible reason to choose us over the alternatives.
  - One specific differentiator — not a feature list, not competitor attacks
  - Depth of the platform vs point solutions (if relevant)
  - Ecosystem / network effects (if relevant)
  - Stability and commitment to the space
  - One compelling, specific reason beats five generic ones.
SECTION 7 — REFERENCE CUSTOMERS (1 slide) — reduce perceived risk.
  - [2–3] references most relevant to THIS account's industry, size, and use case
  ...
  - Confirm every named reference with your AE before including.
SECTION 8 — PRICING OVERVIEW (1 slide) — anchor the investment against the value, not present it in isolation.
...
SECTION 9 — NEXT STEPS (1 slide) — make it easy to say yes and move forward.
...
- Layouts: stats-slide for coverage numbers, two-column for requirement/response pairs, content-slide for narrative sections
...
To build the deck manually in PowerPoint or Google Slides: use the slide plan above as your outline.
...
`${CLAUDE_PLUGIN_ROOT}/references/presales/rfp-library/` — add your best RFP presentation decks here. Naming convention: `[account-or-industry]-[year]-[RFP-topic]-deck.md`

The present command checks this folder for structural patterns and approved content blocks before building the new deck. Good reference decks should include slide titles, key content per slide, and what worked/didn't in the actual presentation.

Confidence-tag all claims in the final deck: 🟢 Confirmed / 🟡 Inferred / 🔴 Unknown. Run `confidence-tagger` skill on the deck content before submitting or presenting.
