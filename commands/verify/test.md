...
1. **Resolve the test stack.** Read the project's `STACK.md` and resolve each component's `test` step per `${CLAUDE_PLUGIN_ROOT}/references/dev/stack-resolution.md` (run from that component's `working_dir`). If there is no `STACK.md`, detect the framework once from project markers and existing tests (e.g. `package.json` → jest/vitest, `pyproject.toml`/`pytest.ini` → pytest, `go test`, `cargo test`, `*_test.rb` → rspec) and recommend `/cc:setup:stack` to persist a manifest. Match the **existing tests' style, directory, and naming** exactly; never introduce a second framework.
