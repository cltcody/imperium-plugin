#!/usr/bin/env bash
# repo-autoupdate.sh — keep configured git repos current at SessionStart, and RAISE
# (never silently apply) when a pulled change needs a reinstall.
#
...
# stays deliberate. It never blocks a session: all problems are surfaced, then it exits 0.
#
# Usage:
  ...
  if git -C "$repo" merge-base --is-ancestor "$local_sha" "$remote_sha" 2>/dev/null; then
    if git -C "$repo" pull --ff-only --quiet 2>/dev/null; then
      ...
      if [[ -f "$repo/install.sh" ]]; then
        echo "     → REINSTALL NEEDED: run  bash \"$repo/install.sh\"  then  /reload-plugins"
      fi
    else
