...
hooks:
  ...
  PostToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "TOOL_INFO=$(jq -r '[.tool_name, .tool_input.command // \"n/a\"] | join(\": \")' | head -1) && mkdir -p .claude/logs && echo \"[$(date -u +%Y-%m-%dT%H:%M:%SZ)] PostToolUse/save-task-list: $TOOL_INFO\" >> .claude/logs/hooks.log"
